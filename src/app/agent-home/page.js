...
const CAROUSEL_ITEM_HEIGHT_MOBILE = 200;
const ASPECT_RATIO_DESKTOP = CAROUSEL_ITEM_WIDTH_DESKTOP / CAROUSEL_ITEM_HEIGHT_DESKTOP;
const ASPECT_RATIO_MOBILE = CAROUSEL_ITEM_WIDTH_MOBILE / CAROUSEL_ITEM_HEIGHT_MOBILE;
...
export default function CompanyInfo() {
  ...
  const resetAutoSlide = useCallback(() => {
    ...
    autoSlideIntervalRef.current = setInterval(handleNext, 5000);
  }, [handleNext]);
  ...
  }, [router, resetAutoSlide, calculateDimensions]);
  ...
  // Calculate carousel item transforms
      ...
      >
        <div className="relative w-full h-full transform-style-preserve-3d transition-transform duration-700">
          {IMAGES.map((image, index) => {
            ...
            const { transform, zIndex, opacity, scale } = getItemTransform(position, isMobile);
